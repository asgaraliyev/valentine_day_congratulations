...
module.exports = class WebGLApp extends EventEmitter {
  ...
  update (dt = 0, time = 0) {
    ...
    const mouseInfluence = 1; // Reduced influence for subtler effect
    this.targetPosition.set(
      this.initialTarget.x + this.mouse.x * mouseInfluence,
      this.initialTarget.y + this.mouse.y * mouseInfluence,
      this.initialTarget.z
    );

    // Smoothly update orbit controls target
    const currentTarget = new THREE.Vector3();
    currentTarget.fromArray(this.controls.target);
    currentTarget.lerp(this.targetPosition, 0.05);
    this.controls.target = currentTarget.toArray();
